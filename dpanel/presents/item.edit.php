...
$doc->msg(__('Подарок успешно изменено'));
...
$form->display();
$doc->act(__('Картинка подарка'), 'item.image.php?id=' . $item['id'] . "&amp;return=" . URL);
$doc->act(__('Удалить подарок'), 'item.delete.php?id=' . $item['id'] . "&amp;return=" . URL);
if (isset($_GET['return']))
$doc->ret(__('Вернуться'), text::toValue($_GET['return']));
else
$doc->ret(__('В категорию'), 'category.php?id=' . $item['id_category']);